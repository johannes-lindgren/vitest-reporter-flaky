...
const writeReport = (report: Report, ouputFile: string, pretty: boolean) => {
...
export type FlakyTestsReporterOptions = {
  /**
   * The file to which the report should be written. If not specified, it will be written to `.vitest-reports/flaky-tests.json`
   */
  outputFile?: string
  /** If true, the report will be pretty-printed. Default is false. */
  pretty?: boolean
}
...
class FlakyTestsReporter implements Reporter {
  ...
  onTestRunEnd(testModules: ReadonlyArray<TestModule>) {
    ...
    console.log(`Test run finished, found ${flakyTests.length} flaky test(s)`)
    if (foundFlakyTests) {
      console.log(
        'Found flaky tests, writing report to',
        this.options.outputFile,
      )
      writeReport(report, this.options.outputFile, this.options.pretty ?? false)
    }
  }
}
